...
class TweetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            try {
                // Store image using Storage facade (to storage/app/public/tweets)
                $path = $request->file('image')->store('tweets', 'public');
                $data['image_path'] = $path;
            } catch (\Exception $e) {
                // Log error but don't fail the tweet creation
                \Log::error('Tweet image upload failed: ' . $e->getMessage());
            }
        }
